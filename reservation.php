<?php
    ...
    if (isset($_REQUEST['submit'])) {
        ...
        if ($result === false) {
            echo "No available rooms for the selected dates or query failed.";
        }
    }
?>
            ...
            if ($result && mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_array($result)) {
                    ...
                    if ($query) {
                        ...
                        echo "Error fetching room details: " . mysqli_error($user->db);
                    }
                }
            } else {
                echo "No available rooms for the selected dates.";
            }
        }
        ?>
    </div>
